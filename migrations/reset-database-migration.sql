-- Complete Reset Database Migration
...
-- Make sure the categories table has the necessary columns
ALTER TABLE categories ADD COLUMN IF NOT EXISTS title VARCHAR(255) NOT NULL DEFAULT name;

-- Create category_members table
...
-- Ensure the boards table has a title column (it's used in the POST route)
ALTER TABLE boards ADD COLUMN IF NOT EXISTS title VARCHAR(255) NOT NULL DEFAULT name;

-- Add title column to boards table if it doesn't exist
ALTER TABLE boards ADD COLUMN IF NOT EXISTS title VARCHAR(255) NOT NULL DEFAULT name;
...
-- Make sure the board_categories table has the correct structure
-- No changes needed to the CREATE TABLE statement as it already exists with the correct structure

-- Ensure the board_categories table has the necessary foreign key constraints
-- These constraints should already exist, but let's make sure they're properly defined
ALTER TABLE "board_categories" ADD CONSTRAINT IF NOT EXISTS "board_categories_board_id_fkey" FOREIGN KEY ("board_id") REFERENCES "boards"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "board_categories" ADD CONSTRAINT IF NOT EXISTS "board_categories_category_id_fkey" FOREIGN KEY ("category_id") REFERENCES "categories"("id") ON DELETE CASCADE ON UPDATE CASCADE;
...
-- Ensure the board_members table has the role column with appropriate default
ALTER TABLE board_members ALTER COLUMN role SET DEFAULT 'VIEWER';
...
-- Create indexes for category_members
...
-- Add foreign key for category_members
...
-- AddForeignKey
ALTER TABLE "integrations" ADD CONSTRAINT "integrations_workspace_id_fkey" FOREIGN KEY ("workspace_id") REFERENCES "workspaces"("id") ON DELETE CASCADE ON
...
CREATE UNIQUE INDEX IF NOT EXISTS "password_reset_tokens_user_id_key" ON "password_reset_tokens"("user_id");
...
CREATE INDEX IF NOT EXISTS "idx_board_categories_category_id" ON "board_categories"("category_id");

CREATE UNIQUE INDEX IF NOT EXISTS "board_categories_board_id_category_id_key" ON "board_categories"("board_id", "category_id");

CREATE INDEX IF NOT EXISTS "idx_board_members_board_id" ON "board_members"("board_id");

CREATE INDEX IF NOT EXISTS "idx_board_members_user_id" ON "board_members"("user_id");

CREATE UNIQUE INDEX IF NOT EXISTS "board_members_board_id_user_id_key" ON "board_members"("board_id", "user_id");
...
CREATE UNIQUE INDEX "invitations_token_key" ON "invitations"("invitations"("token");
